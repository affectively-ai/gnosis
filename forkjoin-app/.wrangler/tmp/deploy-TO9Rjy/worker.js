...
function __wbg_get_imports() {
  const import0 = {
    __proto__: null,
    __wbg___wbindgen_throw_6ddd609b62940d55: /* @__PURE__ */ __name(function (
      arg0,
      arg1
    ) {
    ...
    __wbindgen_cast_0000000000000001: /* @__PURE__ */ __name(function (
      arg0,
      arg1
    ) {
...
var FlowFrame,
  QuantumRuntime,
  FlowFrameFinalization,
  QuantumRuntimeFinalization,
  ...
  '../../node_modules/.bun/gnosis_runtime@file+open-source+gnosis+runtime+pkg/node_modules/gnosis_runtime/gnosis_runtime.js'() {
    ...
    FlowFrame = class _FlowFrame {
      ...
      static create(stream_id, sequence, flags, payload) {
        ...
        const ret = wasm.flowframe_create(
          stream_id,
          sequence,
          flags,
          ptr0,
    ...
    if (Symbol.dispose)
      FlowFrame.prototype[Symbol.dispose] = FlowFrame.prototype.free;
    QuantumRuntime = class {
      ...
      process_frame(encoded_bytes) {
        ...
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.quantumruntime_process_frame(
          this.__wbg_ptr,
          ptr0,
          len0
        );
    ...
    if (Symbol.dispose)
      QuantumRuntime.prototype[Symbol.dispose] = QuantumRuntime.prototype.free;
    ...
    FlowFrameFinalization =
      typeof FinalizationRegistry === 'undefined'
        ...
        : new FinalizationRegistry((ptr) =>
            wasm.__wbg_flowframe_free(ptr >>> 0, 1)
          );
    QuantumRuntimeFinalization =
      typeof FinalizationRegistry === 'undefined'
        ? {
        ...
        : new FinalizationRegistry((ptr) =>
            wasm.__wbg_quantumruntime_free(ptr >>> 0, 1)
          );
...
var getPath = /* @__PURE__ */ __name((request) => {
  ...
  for (; i < url.length; i++) {
    ...
    if (charCode === 37) {
      ...
      return tryDecodeURI(
        path3.includes('%25') ? path3.replace(/%25/g, '%2525') : path3
      );
    ...
    if (/\*$/.test(path3)) {
      ...
      if (method === METHOD_NAME_ALL) {
        Object.keys(middleware).forEach((m) => {
          middleware[m][path3] ||=
            findMiddleware(middleware[m], path3) ||
            findMiddleware(middleware[METHOD_NAME_ALL], path3) ||
            [];
        });
      } else {
        middleware[method][path3] ||=
          findMiddleware(middleware[method], path3) ||
          findMiddleware(middleware[METHOD_NAME_ALL], path3) ||
          [];
      }
      Object.keys(middleware).forEach((m) => {
  ...
  #buildMatcher(method) {
    ...
    [this.#middleware, this.#routes].forEach((r) => {
      const ownRoute = r[method]
        ? Object.keys(r[method]).map((path3) => [path3, r[method][path3]])
        : [];
      if (ownRoute.length !== 0) {
        ...
        routes.push(
          ...Object.keys(r[METHOD_NAME_ALL]).map((path3) => [
            path3,
            r[METHOD_NAME_ALL][path3],
          ])
        );
      }
    });
...
function buildPublicSkillsIndex(options) {
  return {
    ...
    featuredSkills:
      options.featuredSkills && options.featuredSkills.length > 0
        ? [...new Set(options.featuredSkills)]
        : void 0,
    features:
...
function findSkillByUri(skills, uri) {
  ...
  const exact = skills.find(
    (entry) =>
      normalizedUri === entry.uri ||
      normalizedUri === `${entry.uri}/metadata` ||
      normalizedUri === `${entry.uri}/full`
  );
  if (exact) {
    return exact;
  }
  ...
  const candidateSlug =
    segments[segments.length - 1] === 'metadata' ||
...
var QText = class {
  ...
  insert(index, text2) {
    this._content =
      this._content.slice(0, index) + text2 + this._content.slice(index);
...
var GnosisDebugSessionDurableObject = class {
  ...
  createFreshDoc(guid) {
    const doc = new QDoc({
      guid: guid && guid.trim().length > 0 ? guid.trim() : void 0,
    });
    return doc;
  }
...
var GnosisMcpSessionDurableObject = class {
  ...
  ensureInitialized() {
    ...
    this.initPromise = (async () => {
      ...
      this.session =
        (await this.state.storage.get(SESSION_STORAGE_KEY)) ?? null;
      await this.scheduleAlarm();
      this.initialized = true;
    })();
    return this.initPromise;
...
var LogicChainSuperposition = class _LogicChainSuperposition {
  ...
  static fromChains(chains, options = {}) {
    ...
    for (const chain of chains) {
      ...
      if (!Number.isInteger(chain.depth) || chain.depth < 0) {
        throw new Error(
          `chain "${chain.id}" depth must be a non-negative integer`
        );
      }
  ...
  interfere(keyOfState = this.keyOfState) {
    ...
    for (const group of groups.values()) {
      ...
      let strongestProbability =
        representative.amplitude * representative.amplitude;
      for (const chain of group) {
        ...
        if (probability > strongestProbability) {
          ...
          probability === strongestProbability &&
          chain.id < strongestChain.id
        ) {
          strongestChain = chain;
        }
      }
...
var ForkRaceFoldModelChecker = class {
  ...
  async check(model, options = {}) {
    ...
    while (currentLayerIds.length > 0) {
      ...
      for (
        let chunkStart = 0;
        chunkStart < currentLayerIds.length;
        chunkStart += concurrency
      ) {
        const chunkIds = currentLayerIds.slice(
          chunkStart,
          chunkStart + concurrency
        );
        ...
        for (const expansion of expansions) {
          ...
          for (const successor of expansion.successors) {
            ...
            const incomingAmplitude =
              sourceNode.amplitude * successor.amplitude;
            const incomingPhase = multiplyPhase2(
              sourceNode.phase,
              successor.phase
            );
            ...
            if (existingNode) {
              ...
              if (superpositionEnabled) {
                const existingSigned =
                  existingNode.amplitude * existingNode.phase;
                ...
                existingNode.probability =
                  existingNode.amplitude * existingNode.amplitude;
              }
              continue;
            }
    ...
    const violations = eventualResult?.violation
      ? [eventualResult.violation]
      : [];
    return {
  ...
  buildNodeEventuallyProperties(eventual, eventualQuorum) {
    ...
    const quorumPredicates = eventualQuorum.map((property) => ({
      name: property.name,
      test: /* @__PURE__ */ __name(
        (node) => node.quorumSatisfied.has(property.name),
        'test'
      ),
    }));
  ...
  isFairCycle(cycleStateIds, nodes, weakFairness) {
    ...
    for (const fairnessRule of weakFairness) {
      ...
      const enabledEverywhere = cycleStateIds.every((stateId) => {
        const node = nodes.get(stateId);
        return Boolean(
          node && node.enabledActions.has(fairnessRule.actionName)
        );
      });
  ...
  expandNode(node, actions, superposition, eventualQuorum) {
    ...
    const forked = LogicChainSuperposition.seed(node.state, {
      keyOfState,
    }).fork(() =>
    ...
    const interfered =
      superposition.interfere === false ? forked : forked.interfere(keyOfState);
    const distribution = interfered.distribution();
    const resolvedSuccessors = distribution
      .map((entry) => {
  ...
  emitTopologyEvents(node, rawSuccessors, resolvedSuccessors, superposition) {
    ...
    const survivingPaths = new Set(
      resolvedSuccessors.map((successor) => successor.pathId)
    );
    ...
    for (const property of eventualQuorum) {
      ...
      if (property.isGoalKey) {
        const goalKeyResult = superposition.measureQuorum(
          (state) =>
            property.isGoalKey?.(property.keyOfState(state))
              ? '__goal__'
              : '__other__',
          property.threshold
        );
      ...
      if (property.isGoalState) {
        goalSatisfied =
          goalSatisfied &&
          goalChains.some(
            (chain) => property.isGoalState?.(chain.state) === true
          );
      }
      if (goalSatisfied) {
...
function buildGgTemporalModel(program, options = {}) {
  ...
  const initialNodeId =
    options.initialNodeId ?? roots[0] ?? fallbackInitialNode ?? 'root';
  ...
  return {
    ...
    fingerprint: /* @__PURE__ */ __name(
      (state) => `${state.nodeId}:${state.beta1}`,
      'fingerprint'
    ),
    actions: [
      {
...
function buildDefaultGgCheckerOptions(program, defaults = {}) {
  ...
  return {
    ...
    invariants: [
      {
        name: 'beta1_non_negative',
        test: /* @__PURE__ */ __name((state) => state.beta1 >= 0, 'test'),
      },
      {
        name: 'beta1_lt_bound',
        test: /* @__PURE__ */ __name(
          (state) => state.beta1 < maxBeta1Exclusive,
          'test'
        ),
      },
    ],
    eventual: [
      {
        ...
        test: /* @__PURE__ */ __name(
          (state) => terminalNodes.has(state.nodeId),
          'test'
        ),
      },
      {
...
async function checkGgProgram(sourceText, options = {}) {
  ...
  const checkerOptions =
    options.checker ?? buildDefaultGgCheckerOptions(program, options.defaults);
...
var CAPABILITY_PROFILES = {
  ...
  bun: {
    ...
    supported: new Set(NODE_CAPABILITIES),
    description:
      'Bun host with Node-compatible networking and local file access.',
  },
  workers: {
    target: 'workers',
...
function toUri(filePath) {
  return path2.isAbsolute(filePath)
    ? filePath
    : path2.resolve(process.cwd(), filePath);
}
__name(toUri, 'toUri');
...
function applyStrategy(results, vented, strategy) {
  switch (strategy.type) {
    ...
    case 'quorum': {
      ...
      if (bestCount < thresh) {
        throw new Error(`Quorum not met: ${bestCount}/${thresh} agreements`);
      }
      return bestValue;
    }
    case 'merge-all':
    ...
    case 'consensus': {
      ...
      if (strategy.mode === 'constructive') {
        ...
        throw new Error(
          'No outliers found \u2014 destructive interference found full consensus'
        );
      }
    }
    case 'weighted': {
  ...
  for (let i = 0; i < len; i++) {
    ...
    promises[i] =
      s.state === 'completed'
        ...
        : s.state === 'vented'
        ? Promise.reject(new Error(`Stream ${s.id} vented`))
        : executeStream(s);
  }
...
async function search(initialCandidates, config) {
  ...
  for (let i = 0; i < width; i++)
    scored[i] = { candidate: candidates[0], score: 0 };
...
var Superposition = class _Superposition {
  ...
  async search(config) {
    ...
    const settled = await Promise.allSettled(
      streams.map((s) =>
        s.state === 'completed' ? Promise.resolve(s.result) : executeStream(s)
      )
    );
    const candidates = [];
  ...
  then(fn) {
    ...
    const placeholder = fork([() => Promise.resolve(void 0)], null);
...
function computeBuleMeasure(actualBeta1, intrinsicBeta1) {
  ...
  const utilization =
    normalizedIntrinsic === 0
      ? 1
      : Math.min(1, normalizedActual / normalizedIntrinsic);
  let assessment;
  if (deficit === 0) {
    assessment = 'Optimal topology: 0 Bules of waste or opportunity';
  } else if (deficit < 0) {
    assessment = `Over-forked by ${pluralizeBule(
      overforked
    )} beyond intrinsic beta1=${normalizedIntrinsic}`;
  } else if (normalizedActual === 0) {
    assessment = `Sequential bottleneck: ${pluralizeBule(
      waste
    )} of waste/opportunity`;
    ...
    assessment = `Severely underutilized: ${pluralizeBule(
      waste
    )} of waste/opportunity`;
  }
  return {
...
var ReynoldsTracker = class {
  ...
  constructor(capacity, intrinsicBeta1) {
    ...
    this.intrinsicBeta1 =
      intrinsicBeta1 === void 0
        ? void 0
        : Math.max(0, Math.floor(intrinsicBeta1));
...
function toBytes(data, encoding) {
  ...
  if (encoding === 'hex') {
    ...
    if (normalized.length === 0 || normalized.length % 2 !== 0) {
      throw new Error(
        'Hex proof payload must have an even number of characters.'
      );
    }
    if (!isHexString(normalized)) {
...
function parseHaltVerificationOptions(input, props) {
  ...
  return {
    trustedMeasurements:
      trustedMeasurements.length > 0 ? trustedMeasurements : void 0,
    trustedProgramHashes:
      trustedProgramHashes.length > 0 ? trustedProgramHashes : void 0,
    trustedVkHashes: trustedVkHashes.length > 0 ? trustedVkHashes : void 0,
    trustedPublicKeys:
      trustedPublicKeys.length > 0 ? trustedPublicKeys : void 0,
    trustedKeyById:
      Object.keys(trustedKeyById).length > 0 ? trustedKeyById : void 0,
    maxAttestationAgeMs,
    minTcbVersion,
...
function parseEvmProofVerifierConfig(input, props) {
  ...
  return {
    ...
    methodSelector,
    blockTag:
      typeof blockTag === 'string' && blockTag.length > 0 ? blockTag : void 0,
    timeoutMs,
    proofEncoding,
  };
}
...
function hasDelegationConfidentialContext(input, props) {
  if (
    ...
    parseBoolean2(props.confidential, false) ||
    parseBoolean2(props.private, false)
  ) {
    return true;
  }
...
function isCrossBoundarySync(input, props) {
  if (
    ...
    parseBoolean2(props.crossDevice, false) ||
    parseBoolean2(props.crossTenant, false)
  ) {
    return true;
  }
  const tenantScope =
...
function isPrivateMaterialization(input, props) {
  if (
    ...
    parseBoolean2(props.private, false) ||
    parseBoolean2(props.userPrivate, false)
  ) {
    return true;
  }
  const visibility =
      ...
      const verification = await verifyGranularUcan({
        ...
        verifyOptions: {
          ...
          requiredCapabilities:
            requiredCapabilities.length > 0 ? requiredCapabilities : void 0,
        },
      });
    ...
    while (currentNodeId) {
      ...
      if (edge.type === 'OBSERVE') {
        ...
        const entangleEdges = activeAst.edges.filter(
          (e) =>
            e.type === 'ENTANGLE' &&
            e.sourceIds.some((sid2) =>
              edge.targetIds.map((t) => t.trim()).includes(sid2.trim())
            )
        );
      ...
      if (
        ...
        const workFns = activeTargets.map((id, index) => {
          ...
          return async () => {
            ...
            return {
              path: tid,
              value: `Simulated Result from ${tid}`,
              time: 0,
            };
          };
        });
        ...
        const tunnelEdge = activeAst.edges.find(
          (e) =>
            e.type === 'TUNNEL' &&
            e.sourceIds.some((sid2) =>
              activeTargets.map((t) => t.trim()).includes(sid2.trim())
            )
        );
        if (tunnelEdge) {
          execLogs.push(
        ...
        if (collapseEdge.type === 'RACE') {
          execLogs.push(
            `   Racing paths: [${collapseEdge.sourceIds.join(', ')}]`
          );
          ...
          execLogs.push(
            `   Folding paths: [${collapseEdge.sourceIds.join(', ')}]`
          );
          ...
          execLogs.push(
            `   Folded result: ${JSON.stringify(currentPayload).substring(
              0,
              50
            )}...`
          );
    ...
    return {
      ...
      principal:
        typeof executionAuth.principal === 'string'
          ...
          : void 0,
      token:
        typeof executionAuth.token === 'string' ? executionAuth.token : void 0,
      capabilities,
    };
  }
  ...
  async initializeWasmRuntime() {
    ...
    try {
      const module = await Promise.resolve().then(
        () => (init_gnosis_runtime(), gnosis_runtime_exports)
      );
...
var GNOSIS_MCP_TOOLS = [...PUBLIC_TOOL_DEFINITIONS, ...AUTH_TOOL_DEFINITIONS];
var TOOL_BY_NAME = new Map(GNOSIS_MCP_TOOLS.map((tool) => [tool.name, tool]));
var PUBLIC_TOOL_NAMES = new Set(
  PUBLIC_TOOL_DEFINITIONS.map((tool) => tool.name)
);
var AUTH_TOOL_NAMES = new Set(AUTH_TOOL_DEFINITIONS.map((tool) => tool.name));
  ...
  if (!response.ok || payload.ok === false) {
    const message =
      typeof payload.message === 'string'
        ...
        ? payload.error
        : `Debug session request failed: ${path3}`;
          ...
          text: JSON.stringify(
            {
              ...
              source: {
                ...
                aeonContainer:
                  'https://github.com/affectively-ai/aeon-container',
