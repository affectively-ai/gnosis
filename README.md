...
| Built-in topological complexity tracking | Compiler/runtime track superposition pressure (`beta1`), compute a “Buley” complexity metric, and now surface frontier-fill steering diagnostics as a Wallace number measured in `wally`, plus runtime wall/CPU telemetry in micro-Charleys. | [compiler.ts:61](open-source/gnosis/src/betty/compiler.ts:61), [compiler.ts:217](open-source/gnosis/src/betty/compiler.ts:217), [analysis.ts:131](open-source/gnosis/src/analysis.ts:131) |
...
*   **Learned Fold-Boundary Benchmarks**: parameter-matched `.gg` modules in [`examples/benchmarks/`](./examples/benchmarks/README.md) differ only in fold strategy (`linear`, `winner-take-all`, `early-stop`) and drive the seeded Chapter 17 training benchmark.
...
- `wallaceNumber`: the primary frontier pessimism / underfill statistic
- `frontierDeficit`: descriptive alias for the same underfill metric
- `wally`: short-form alias and unit label for Wallace output
- `telemetry.wallMicroCharleys` / `telemetry.cpuMicroCharleys`: observed wall and CPU execution time
...
The two warm-up suites are meant to be read together: one shows the shape that gets better after first use, the other shows how the same mechanism degrades when the race winner, fold policy, or feedback channel is wrong.
