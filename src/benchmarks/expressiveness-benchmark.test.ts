...
const PROBLEMS = [
  {
    ...
    erlang: 18,
    ts: 22,
    primitives: ['FORK', 'PROCESS', 'RACE', 'VENT'],
  },
  {
    name: 'Consensus',
    gnosis: 6,
    ...
    erlang: 22,
    ts: 28,
    primitives: ['FORK', 'PROCESS', 'FOLD', 'RACE', 'VENT'],
  },
  {
    name: 'Load Balancer',
    gnosis: 7,
    ...
    erlang: 35,
    ts: 38,
    primitives: ['FORK', 'RACE', 'PROCESS', 'VENT'],
  },
  {
    ...
    erlang: 45,
    ts: 52,
    primitives: ['RACE', 'PROCESS', 'FOLD', 'VENT'],
  },
  {
    name: 'Saga Pattern',
    gnosis: 9,
  ...
  },
  {
    ...
    erlang: 28,
    ts: 32,
    primitives: ['FORK', 'PROCESS', 'FOLD', 'RACE'],
  },
];
  ...
  test('FORK and FOLD appear together (structured concurrency)', () => {
    for (const p of PROBLEMS) {
      if (p.primitives.includes('FORK')) {
        ...
        const hasJoin =
          p.primitives.includes('RACE') || p.primitives.includes('FOLD');
