        ...
        const output = await renderWithTopologyCompat(
            {
                scene: {
                    branches: [
                        { id: 'geometry', estimatedCostMs: 3.2, vertices: 80_000, drawCalls: 50 },
                        { id: 'post', estimatedCostMs: 5.1, vertices: 20_000, drawCalls: 18 },
                        { id: 'global-illumination', estimatedCostMs: 13.5, vertices: 120_000, drawCalls: 64 }
                    ],
                    budgetMs: 8,
                    collapse: 'fold'
                }
        ...
        const output = await renderWithTopologyCompat(
            {
                scene: {
                    branches: [
                        { id: 'high-fidelity', estimatedCostMs: 6.0, quality: 2, vertices: 140_000, drawCalls: 90 },
                        { id: 'fast-path', estimatedCostMs: 2.0, quality: 1, vertices: 30_000, drawCalls: 20 }
                    ],
                    budgetMs: 7
                }
            },
