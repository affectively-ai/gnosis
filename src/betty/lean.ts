...
function sanitizeLeanId(name: string): string {
  return name.replace(/[^a-zA-Z0-9_]/g, '_');
}

function topologyTypeToLean(typeStr: string): string {
  // Map formatted type strings to Lean constructors.
  const mapping: Record<string, string> = {
    'Json(integer)': 'TopologyType.json JsonSchemaKind.integer',
    'Json(number)': 'TopologyType.json JsonSchemaKind.number',
    'Json(string)': 'TopologyType.json JsonSchemaKind.string_',
    'Json(boolean)': 'TopologyType.json JsonSchemaKind.boolean',
    'Json(null)': 'TopologyType.json JsonSchemaKind.null_',
    'Json(any)': 'TopologyType.json JsonSchemaKind.any',
    'Bytes': 'TopologyType.bytes',
    'Product{..}': 'TopologyType.product 0 true',
    'Product{}': 'TopologyType.product 0 false',
    '?': 'TopologyType.unknown',
  };

  if (mapping[typeStr]) return mapping[typeStr];

  // Handle Stream<X>
  const streamMatch = typeStr.match(/^Stream<(.+)>$/);
  if (streamMatch) {
    return `TopologyType.stream (${topologyTypeToLean(streamMatch[1])})`;
  }

  // Handle Option<X>
  const optionMatch = typeStr.match(/^Option<(.+)>$/);
  if (optionMatch) {
    return `TopologyType.option_ (${topologyTypeToLean(optionMatch[1])})`;
  }
...
/**
 * Generate a Lean 4 proof certificate for semantic type compatibility.
 *
 * Given a SemanticCompatibilityResult, generates Lean 4 theorems proving
 * that the compatible edges are type-safe.
 */
export function generateSemanticLeanCertificate(
  topologyName: string,
  edgeChecks: Array<{
    sourceNode: string;
    targetNode: string;
  ...
  const lines: string[] = [];
  lines.push(`-- Lean 4 semantic compatibility certificate for topology: ${topologyName}`);
  lines.push(`-- Generated by Betty Phase 8`);
  lines.push(`-- This certificate proves that all compatible edges preserve semantic types.`);
  lines.push('');
  lines.push('import ForkRaceFoldTheorems.SemanticCompatibility');
  lines.push('');

  for (const check of edgeChecks) {
    if (!check.compatible) continue;

    const safeSrc = sanitizeLeanId(check.sourceNode);
    const safeTgt = sanitizeLeanId(check.targetNode);

    lines.push(`/-- Edge ${check.sourceNode} → ${check.targetNode}: ${check.sourceType} → ${check.targetType} -/`);
    lines.push(`theorem edge_${safeSrc}_to_${safeTgt}_compatible :`);
    lines.push(`    topologyTypeCompat`);
    lines.push(`      (${topologyTypeToLean(check.sourceType)})`);
    lines.push(`      (${topologyTypeToLean(check.targetType)})`);
    lines.push(`    = TypeCompat.compatible := by`);
    lines.push(`  simp [topologyTypeCompat, jsonSchemaCompat]`);
    lines.push('');
  }

  const compatCount = edgeChecks.filter(e => e.compatible).length;
  const totalCount = edgeChecks.length;
  lines.push(`-- Summary: ${compatCount}/${totalCount} edges proved compatible`);
