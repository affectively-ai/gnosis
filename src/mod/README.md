...
- [resolver.gg](./resolver.gg): GG-native resolver topology for specifier kind and filesystem-candidate routing (`branch(bare|filesystem) -> prepare-path -> branch(exact|search)`), used by `createFilesystemModuleResolver()`.
- [loader.ts](./loader.ts): native `.gg` / `.mgg` import/export parsing, filesystem-backed and lockfile-backed module resolution, GG-driven loading, cycle detection, merged effect-contract summaries, AST merge/lowering, and canonical merged-source rendering for CLI/runtime consumers.
