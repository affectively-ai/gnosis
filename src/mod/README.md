...
- [loader.gg](./loader.gg): GG-native module loading pipeline with native case routing for format and import presence (`detect-format -> branch(gg|mgg) -> parse -> branch(imports|none) -> resolve-specifiers -> load-imports -> validate-imports -> compile -> determine-exports -> merge -> validate-exports -> assemble`) used by the host shim for unified `.gg` / `.mgg` orchestration.
