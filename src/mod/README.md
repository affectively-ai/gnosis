...
- [loader.gg](./loader.gg): GG-native module loading pipeline with native case routing for import presence (`parse -> branch(imports|none) -> resolve-specifiers -> load-imports -> validate-imports -> compile -> determine-exports -> merge -> validate-exports -> assemble`) used by the host shim for `.mgg` orchestration.
