...
- [loader.ts](./loader.ts): native `.mgg` import/export parsing, filesystem-backed and lockfile-backed module resolution, AST merge/lowering, and canonical merged-source rendering for CLI/runtime consumers.
- [loader.test.ts](./loader.test.ts): Regression tests for `.mgg` parsing, re-export modules, bare-specifier dependency loading, import validation, and merged topology loading.
